<nav x-data="{ mobileMenuOpen: false }" class="bg-[#1F4E79] dark:bg-slate-900 text-white shadow-lg sticky top-0 z-50 transition-colors duration-300 border-b border-transparent dark:border-slate-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex-shrink-0 flex items-center gap-3">
                ...
                <span class="px-2.5 py-1 bg-blue-800/50 dark:bg-blue-900/40 rounded-lg text-xs font-semibold tracking-widest text-blue-200 border border-blue-700 dark:border-blue-800/50">DONATUR</span>
            </div>
            ...
            <div class="hidden md:flex items-center space-x-1">
                ...
                <a href="{{ route('donatur.dashboard') }}" class="px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('donatur.dashboard') ? 'bg-white/10 dark:bg-slate-800' : 'hover:bg-white/10 dark:hover:bg-slate-800' }} transition">Dashboard</a>
                @if(!request()->routeIs('donatur.dashboard'))
                    <a href="{{ route('donatur.riwayat') }}" class="px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('donatur.riwayat') ? 'bg-white/10 dark:bg-slate-800' : 'hover:bg-white/10 dark:hover:bg-slate-800' }} transition">Riwayat Donasi</a>
                @endif
    ...
    <div x-show="mobileMenuOpen" x-collapse x-cloak class="md:hidden border-t border-blue-800/50 dark:border-slate-800 bg-[#163859] dark:bg-slate-900">
        <div class="px-2 pt-2 pb-3 space-y-1">
            ...
            <a href="{{ route('donatur.dashboard') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('donatur.dashboard') ? 'bg-white/10 dark:bg-slate-800' : 'hover:bg-white/10 dark:hover:bg-slate-800' }} transition">Dashboard</a>
            @if(!request()->routeIs('donatur.dashboard'))
                <a href="{{ route('donatur.riwayat') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('donatur.riwayat') ? 'bg-white/10 dark:bg-slate-800' : 'hover:bg-white/10 dark:hover:bg-slate-800' }} transition">Riwayat Donasi</a>
            @endif
