...
<div class="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col justify-between transition-colors duration-300">
    ...
    <div class="max-w-3xl w-full mx-auto px-4 py-8 flex-grow">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-8 transition-colors">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Buat Laporan Distribusi Baru</h2>
            <p class="text-sm text-gray-400 dark:text-gray-500 mb-6 border-b border-gray-100 dark:border-slate-700 pb-4">Isi rincian penggunaan dana donasi kebencanaan secara terperinci.</p>
            ...
            <form method="POST" action="{{ route('admin.laporan-distribusi.store') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    @error('bencana_id')
                        <p class="mt-1.5 text-xs text-red-650 dark:text-red-400 flex items-center gap-1">
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 text-sm font-bold">
                            Rp
                        </span>
                        <input id="jumlah_disalurkan" name="jumlah_disalurkan" type="number" 
                               ...
                               class="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl text-sm focus:outline-none focus:border-[#1F4E79] dark:focus:border-blue-500 focus:ring-2 focus:ring-[#1F4E79]/20 dark:focus:ring-blue-500/20 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 transition {{ $errors->has('jumlah_disalurkan') ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : '' }}">
                    </div>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Masukkan nominal rupiah tanpa titik/koma. Nilai default adalah 0.</p>
                ...
                <div>
                    ...
                    <textarea id="rincian_penggunaan" name="rincian_penggunaan" rows="5" required
                              ...
                              class="w-full px-4 py-2.5 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl text-sm focus:outline-none focus:border-[#1F4E79] dark:focus:border-blue-500 focus:ring-2 focus:ring-[#1F4E79]/20 dark:focus:ring-blue-500/20 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 transition {{ $errors->has('rincian_penggunaan') ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : '' }}">{{ old('rincian_penggunaan') }}</textarea>
                ...
                <div>
                    <label for="bukti_distribusi" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                        Unggah Bukti Distribusi <span class="text-gray-400 dark:text-gray-500 font-normal">(opsional)</span>
                    </label>
                    <input id="bukti_distribusi" name="bukti_distribusi" type="file" accept=".pdf,.jpg,.jpeg,.png"
                           class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-blue-50 dark:file:bg-blue-900/30 file:text-[#1F4E79] dark:file:text-blue-400 hover:file:bg-blue-100 dark:hover:file:bg-blue-900/50 border border-gray-300 dark:border-slate-600 rounded-xl p-2 bg-gray-50 dark:bg-slate-700 transition {{ $errors->has('bukti_distribusi') ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : '' }}">
                    <p class="text-[10px] text-gray-450 dark:text-gray-500 mt-1">Format dokumen yang diizinkan: PDF, JPG, JPEG, PNG (Ukuran maksimal 5MB).</p>
